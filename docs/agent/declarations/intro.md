...
```mermaid
flowchart TD
    A[Agent saisit la déclaration] --> B[SIGFID calcule l'impôt<br/>paramètres depuis la BDD]
    B --> C{Agent valide ?}
    C -->|Oui| D[Déclaration enregistrée<br/>audit trail]
    C -->|Non — correction| A
    D --> E[Création de la dette<br/>dans le compte impôts]
    E --> F[Notification au contribuable]
    F --> G[Contribuable paie]
    G --> H[Agent enregistre le paiement]
    H --> I[Compte impôts soldé]
